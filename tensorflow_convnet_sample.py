...
def class_error_fn(model_output_class, target_class):

    with tf.variable_scope("class_error"):

        accuracy = tf.metrics.accuracy(predictions=model_output_class[0],
                                       labels=target_class)

        tf.summary.scalar('sum_class_error', tf.reduce_sum(accuracy))
...
def model(data_pipeline, model, class_loss_fn, reg_loss_fn):

    # Insert your data unpacking here, which may be nothing.
    (altitude_image,
     latitude_image,
     longitude_image,
     class_label,
    ...
    with tf.variable_scope("loss"):
        ...
        class_error = class_error_fn(model_output_class, class_label)

        class_error_print_op = tf.print("\nclass_error: ", class_error)
